  ...
  if(SM.getFileID(Decl->getLocation()) == SM.getMainFileID()) {
    ...
    if(llvm::isa<clang::VarDecl>(Decl)) {
      ...
      if(vd->getType().getBaseTypeIdentifier()) {
          // we check for interval-defn's that are not the autogenerated loopvariables
        if(vd->getType().getBaseTypeIdentifier()->getName() == "interval" && vd->getName()!="__k_loopvar__") {
