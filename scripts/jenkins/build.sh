...
if [ "$ENABLE_GPU" = true ]; then
  cmake ${CMAKE_ARGS} -DGTCLANG_BUILD_EXAMPLES_WITH_GPU=ON -DCTEST_CUDA_SUBMIT=ON -DGTCLANG_SLURM_RESOURCES="${SLURM_RESOURCES[@]}" -DGTCLANG_SLURM_PARTITION=${SLURM_PARTITION} -DSLURM_RESOURCES="${SLURM_RESOURCES[@]}" -DSLURM_PARTITION=${SLURM_PARTITION} -DGPU_DEVICE=${GPU_DEVICE} ../
else
  cmake ${CMAKE_ARGS} ../
fi
